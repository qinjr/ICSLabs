...
long_t compute_alu(alu_t op, long_t argA, long_t argB)
{
    ...
    switch(op) {
        case A_ADD:
            ...
            break;
        case A_SUB:
            val = argB - argA;//which is in the front
            break;
        case A_AND:
            val = argA & argB;
            break;
...
stat_t nexti(y86sim_t *sim)
{
    ...
    byte_t regs = 0;
    reg_t regA = {"init", REG_NONE};
    reg_t regB = {"init", REG_NONE};
    if ((icode != I_HALT) && (icode != I_NOP) && (icode != I_JMP) && (icode != I_CALL) && (icode != I_RET)) {
        get_byte_val(sim->m, next_pc, &regs);
        regA = reg_table[GET_REGA(regs)];
        regB = reg_table[GET_REGB(regs)];
        next_pc ++;
    }
    ...
    switch (icode) {
      ...
      case I_RRMOVL:  /* 2:x regA:regB */
        if (cond_doit(sim->cc, ifun)) {
            set_reg_val(sim->r, regB.id, get_reg_val(sim->r, regA.id));
            sim->pc = next_pc;
            break;
        }
        else {
      ...
      case I_IRMOVL: /* 3:0 F:regB imm */
        set_reg_val(sim->r, regB.id, imm);
        sim->pc = next_pc;
        break;
      case I_RMMOVL: /* 4:0 regA:regB imm */
        set_long_val(sim->m, get_reg_val(sim->r, regB.id) + imm, get_reg_val(sim->r, regA.id));
        sim->pc = next_pc;
        break;
      case I_MRMOVL: /* 5:0 regB:regA imm */
        if (get_long_val(sim->m, get_reg_val(sim->r, regB.id) + imm, &int_get)) {
            set_reg_val(sim->r, regA.id, int_get);
            sim->pc = next_pc;
            break;
        }
        else {
            err_print("PC = 0x%x, Invalid data address 0x%x", sim->pc, get_reg_val(sim->r, regB.id) + imm);
            return STAT_ADR;
            break;
        }
      case I_ALU: /* 6:x regA:regB */
        value_alu = compute_alu(ifun, get_reg_val(sim->r, regA.id), get_reg_val(sim->r, regB.id));
        set_reg_val(sim->r, regB.id, value_alu);
        sim->cc = compute_cc(ifun, get_reg_val(sim->r, regA.id), get_reg_val(sim->r, regB.id), value_alu);
        sim->pc = next_pc;
        break;
      case I_JMP: /* 7:x imm */
        if (cond_doit(sim->cc, ifun)) {
      ...
      case I_CALL: /* 8:x imm */
        ...
        if (set_long_val(sim->m, get_reg_val(sim->r, REG_ESP), next_pc)){}
            ...
            err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, get_reg_val(sim->r, regB.id) - 4);
            return STAT_ADR;
        }
        //change pc
        sim->pc = imm;
        break;
      case I_RET: /* 9:0 */
        get_long_val(sim->m, get_reg_val(sim->r, REG_ESP), &int_get);
        sim->pc = int_get;
        set_reg_val(sim->r, REG_ESP, get_reg_val(sim->r, REG_ESP) + 4);
        break;
      case I_PUSHL: /* A:0 regA:F */
        if (set_long_val(sim->m, get_reg_val(sim->r, REG_ESP) - 4, get_reg_val(sim->r, regA.id))) {
            set_reg_val(sim->r, REG_ESP, get_reg_val(sim->r, REG_ESP) - 4);
            sim->pc = next_pc;
            break;
        }
