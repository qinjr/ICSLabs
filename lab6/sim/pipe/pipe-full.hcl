...
## Description:
## 1.Implement iaddl, leave to decrease the number of instruction
## 2.Improve pipeline to "load forwarding"
##	mrmovl (%ebx), %esi
##	rmmovl %esi, (%ecx)
##   don't need bubble anymore
## 3.When the instruction sequence is like below
##	andl %ra, %ra
##	jle 
##   jle here can predict branch 100% accurate
## 4.In jump table, I make the ret instruction just have 1 bubble, which
## is because load/use hazard, but not the bubbles after ret
## 	pushl
##   	ret
## just 1 bubble, and the bubble is between pushl and ret, not
## after ret
####################################################################
...
int f_pc = [
	...
	E_icode == IOPL && E_ifun == 2 && 
	E_srcA == E_srcB && E_valA > 0 : D_valP;
	# Mispredicted branch.  Fetch at incremented PC
	M_icode == IJXX && !M_Cnd && 
	!(M_ifun == 1 && W_icode == IOPL && W_ifun == 2): M_valA;
	# pushl ret
...
int e_valA = [
	E_srcA == M_dstM && E_icode in { IRMMOVL } : m_valM;
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd &&
	!(E_ifun == 1 && M_icode == IOPL && M_ifun == 2 && M_valE > 0)) ||
	# Stalling at fetch while ret passes through pipeline
	# but not condition for a load/use hazard
	#IRET in { D_icode, E_icode, M_icode };
	!(E_icode in { IMRMOVL, IPOPL } && E_dstM in { d_srcA, d_srcB } && !D_icode in { IRMMOVL }) &&
	 (D_icode == IRET && E_icode != IPUSHL || 
	E_icode ==  IRET && M_icode != IPUSHL  || 
	M_icode == IRET && W_icode != IPUSHL);
